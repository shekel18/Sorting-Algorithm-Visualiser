...
const App: React.FC = () => {
  ...
  const generateArray = useCallback((size: number, distribution: ArrayDistribution = 'random', customArray?: number[]) => {
    ...
    if (customArray) {
      ...
      setContenderArray([...customArray]);
      originalArrayRef.current = [...customArray];
  ...
  const clearTimeoutAnimation = () => {
    ...
    if (contenderTimeoutRef.current) {
      clearTimeout(contenderTimeoutRef.current);
  ...
  const onSortCompletion = useCallback((isContender: boolean = false) => {
      if (!isContender) {
          ...
          setContenderSortedIndices(Array.from(Array(contenderArray.length).keys()));
      }
      
      if ((!contender && !isContender) || (isSorted && contenderIsSorted) || (isSorted && isContender) || (contenderIsSorted && !isContender)) {
          // Sync logic if needed
      }
  }, [array.length, contenderArray.length, contender, isSorted, contenderIsSorted, showCompletionToast]);
  ...
  const handleStop = useCallback(() => {
    ...
    setContenderArray([...originalArrayRef.current]);
    setActiveIndices([]);
    setSwapIndices([]);
    setSortedIndices([]);
    setContenderActiveIndices([]);
  ...
  const prepareForSort = () => {
    ...
    animationsRef.current = getAnimationsFor(algorithm, array);
    if (contender) {
      contenderAnimationsRef.current = getAnimationsFor(contender, array);
    }
  ...
  const handleStartSort = useCallback((mode: 'normal' | 'turbo') => {
    ...
    prepareForSort();
    setSortingMode(mode);
  }, [sortingMode, isSorted, array, algorithm, contender, sortDirection, volume, isMuted]);
  ...
  }, [sortingMode, isPaused, algorithm, contender]);
  ...
  const performStep = (isContender: boolean) => {
    ...
    if (type === 'compare' || type === 'pivot') {
      ...
      if (isContender) {
        ...
        setActiveIndices([]);
        setDebugVars(prev => ({...prev, i: idx1, j: idx2}));
        setArray(prev => {
          const next = [...prev];
          [next[idx1], next[idx2]] = [next[idx2], next[idx1]];
          return next;
  ...
  }, [array, speed, sortedIndices.length, algorithm, sortingMode, isPaused]);
  ...
  }, [contenderArray, speed, contenderSortedIndices.length, contender, sortingMode, isPaused]);
  ...
  }, [sortingMode, isPaused, animate, animateContender]);
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      ...
      <ControlPanel
        ...
        onGenerateArray={(dist) => generateArray(arraySize, dist)}
        onCustomArray={(custom) => generateArray(custom.length, 'random', custom)}
      ...
      <main className="flex-grow flex flex-col lg:flex-row items-stretch justify-center p-2 sm:p-4 gap-4 relative">
        <div className={`flex-grow flex flex-col items-center justify-center relative transition-all duration-500 ${contender ? 'lg:flex-row gap-4' : ''}`}>
          <div className={`relative transition-all duration-500 h-full w-full ${contender ? 'lg:w-1/2' : 'w-full'}`}>
            <Visualizer
              ...
              isResetting={isResetting} theme={theme} algorithm={algorithm} debugVars={debugVars} stats={stats} label="Primary"
            />
          </div>
          {contender && (
            <div className="relative h-full w-full lg:w-1/2 animate-in slide-in-from-right-10 duration-500">
               <Visualizer
                ...
                isResetting={isResetting} theme={theme} algorithm={contender} stats={contenderStats} label="Contender" isContender
              />
            </div>
          )}
        </div>
        ...
        {isInfoOpen && (
          <AlgorithmInfo algorithm={algorithm} onClose={() => setIsInfoOpen(false)} onSelectAlgorithm={setAlgorithm} highlightedLine={currentLine} />
        )}
